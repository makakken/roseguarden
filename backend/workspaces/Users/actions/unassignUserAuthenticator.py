...
class UnassignUserAuthentictor(Action):
    ...
    def handle(self, action, user, workspace, actionManager ):
        ...
        notification_action = webclientActions.NotificationAction.generate("Assign authenticator was succesful.", "success")
        
        if user is None:
            notification_action = webclientActions.NotificationAction.generate("You have to be logged in to do this action.", "error")
            return 'success', [notification_action], {'succeed' : False, 'message' : "You have to be logged in to do this action."}            

        logManager.info("Request for authenticator unassign for {} by {}".format(action.userId, user.email))

        if user_to_unassign is None:
            notification_action = webclientActions.NotificationAction.generate("Failed to unassign authenticator to user.", "error")
            return 'success', [notification_action], {'succeed' : False, 'message' : "Failed to unassign authenticator to user."}            
        ...
        if user.email != user_to_unassign.email:
            if user_to_unassign.authenticator_status is not UserAuthenticatorStatus.UNSET:   
                notification_action = webclientActions.NotificationAction.generate("Failed to unassign authenticator to user", "error")
                return 'success', [notification_action], {'succeed' : False, 'message' : "Failed to unassign authenticator."}            
