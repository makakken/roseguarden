<template>
  ...
  >
    <v-toolbar color="primary" dark >
      <img src="../static/fabba_logo.png" height="36" alt="">
      <v-toolbar-title href="/" class="ml-0 pl-3">
        <v-btn to="/" text><span class="hidden-sm-and-down">roseguarden</span> </v-btn>
      </v-toolbar-title>
    </v-toolbar>
    <vue-perfect-scrollbar v-bind:class="['drawer-menu--scroll', submenu ? 'submenustyle' : '' ]" :settings="scrollSettings">
      ...
      <v-list v-else dense expand>
        <v-list-item v-if="submenu" :to="item.href">
          ...
          <v-list-item-content @click="click()">
            <v-list-item-title style="font-size: 1.5em;">{{submenuname}}</v-list-item-title>
          </v-list-item-content>
        </v-list-item>        
